...
module.exports = async (memberID, time, reason) => {
    if (!memberID) throw new Error('[Mocy] Error: Member is not defined in remind function')
    if (!time) throw new Error('[Mocy] Error: Time is not defined in remind function')
    if (!reason) throw new Error('[Mocy] Error: Reason is not defined in remind function')
    if (!ms(time.toString())) throw new Error('[Mocy] Error: Time is invalid in remind function')
    ...
    data.save().catch(e => console.log("[Mocy] Error: saving remind to db"))
};
...
module.exports.startRemind = async (client) => {
    if (!client) throw new Error('[Mocy] Error: Client is not defined in remind function')
    setInterval(() => {
